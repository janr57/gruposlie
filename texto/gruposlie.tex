...
de su eje. Más adelante se dará una definición sencilla.
...
\begin{align*}
  ...
  &= \text{cof}\,(B_{11})
    ...
    \begin{pNiceArray}{rr}
      ...
      \CodeAfter
      \tikz \draw[red,line width=10pt,opacity=0.15] (1.5-|1.1) -- (1.5-|last);
      \tikz \draw[red,line width=30pt,opacity=0.15] (1.5|-1.1) -- (1.5|-last);
    \end{pNiceArray}
    = \text{det}\,(\cos\alpha)
    = \cos\alpha\\
  A_{12}
  &= \text{cof}\,(B_{12})
    ...
    \begin{pNiceArray}{rr}
      ...
      \CodeAfter
      \tikz \draw[red,line width=10pt,opacity=0.15] (1.5-|1.1) -- (1.5-|last);
      \tikz \draw[red,line width=30pt,opacity=0.15] (2.5|-1.1) -- (2.5|-last);
    \end{pNiceArray}
    = - \text{det}\,(-\sin\alpha)
    = \sin\alpha\\
  A_{21}
  &= \text{cof}\,(B_{21})
    ...
    \begin{pNiceMatrix}
      ...
      \CodeAfter
      \tikz \draw[red,line width=10pt,opacity=0.15] (2.5-|1.1) -- (2.5-|last);
      \tikz \draw[red,line width=30pt,opacity=0.15] (1.5|-1.1) -- (1.5|-last);
    \end{pNiceMatrix}
    = -\text{det}\,(\sin\alpha)
    = -\sin\alpha\\
...
\begin{itemize}
\item La matriz de rotación\footnotemark{} depende de la dimensión del espacio; cuando
  vale 2, es\footnotetext{Matriz de rotación pasiva.}
  ...
  trigonométricas, la volveremos a obtener a partir del concepto de
  \emph{generador de una rotación}, que se introducirá en la sección siguiente
  \ref{sec:defgrupolie}.
  
\item Las matrices de rotación $\mmm{R} (\alpha)$ dependen de un parámetro, el ángulo,
  que es una variable continua.

\item Las matrices que cumplen que su inversa es igual a la
  traspuesta, como en \eqref{eq:gli-R_ortogonal}, se dice que son \emph{ortogonales}.
  Además, para que se trate de rotaciones, tienen que tener un determinante igual a $+1$,
  ver la ecuación~\eqref{eq:gli-detRuno}. Estas matrices suelen presentarse en las
  transformaciones entre bases de espacios vectoriales.
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[%
    ...
    vectorrotado/.style={vector, draw=\colorVrot},
    textooriginal/.style={\colorTorig},
    textorotado/.style={\colorTrot},
    pcirculo/.style={fill=\colorPorig, draw=black},
    pprimacirculo/.style={\colorProt, draw=black},
    ...
    % Ángulo \varepsilon
%    \path (P') -- (O) -- (P) pic
%    [{Latex[width=2.5pt,length=3.7pt]}-,draw=black!50,fill=\colorAng,
%    "\footnotesize $\varepsilon$",angle radius=27pt, angle eccentricity=0.75]
%    {angle = P'--O--P};
    \path (P') -- (O) -- (P) pic
...
\[
  ...
  ;\hspace{0.2em} \mmm{R}''' (0) = \dfrac{N\,(N-1)\,(N-2)\,\mmm{G}^3}{N^3}
  = \mmm{G}^3
\]
}
y así sucesivamente.
