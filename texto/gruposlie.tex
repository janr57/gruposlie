...
\begin{figure}[ht]
  ...
  \begin{minipage}{0.40\linewidth}
    ...
    % Fondo
    \pgfmathsetmacro{\HORZ}{0.56}
    \pgfmathsetmacro{\VERT}{0.24}
    \pgfmathsetmacro{\MOD}{\HORZ^2 + \VERT^2}
...
%      {angle = xfin--O--P};
  ...
  \begin{minipage}{0.45\linewidth}
    ...
    \pgfmathsetmacro{\ANGROTPLUSNINETY}{\ANGROT + 90}
     % Fondo
    \pgfmathsetmacro{\HORZ}{0.45}
    \pgfmathsetmacro{\VERT}{0.40}
    \pgfmathsetmacro{\MOD}{\HORZ^2 + \VERT^2}
    \pgfmathsetmacro{\ANGSD}{atan(\VERT / \HORZ)}
    \pgfmathsetmacro{\ANGII}{\ANGSD + 180.0}
    % 
...
lo indicamos en rojo y, en este caso, el menor complementario es un determinante formado
por un único elemento, que está en negro\footnotemark{}
\footnotetext{Las barras verticales en estas expresiones representan determinantes,
  no valor absoluto.}
\begin{align*}
  A_{11} &= (-1)^{1+1}\,
  \begin{vmatrix}
...
Abordaremos este apartado de dos formas; en la primera, desarrollaremos los cálculos con todo detalle y, en la segunda, seguiremos un razonamiento aún más sofisticado y potente.
...
Recalcamos que hemos llegado a esta conclusión sin presuponer la dimensión del espacio, por lo que hemos demostrado que se debe cumplir sea cual sea esta.
...
\]
que es el grupo de matrices ortogonales $2\times 2$, que describen la rotación en el plano real y forman un grupo de Lie.
...
Si un parámetro $\varepsilon$ es infinitesimal, en primera aproximación, se considera que su cuadrado es despreciable (si $\varepsilon$ es infinitesimal, entonces $\varepsilon^2\approx 0$).
...
La matriz de rotación infinitesimal $\mmm{I} + \mmm{A}$, por ser matriz de rotación, debe ser ortogonal
\[
  \mmm{R}^\trasp \mmm{R} = \mmm{I}
\]
\[
...
La expresión~\eqref{eq:gli-A_antisimetrica} que se obtiene, define a la matriz \mmm{A} como antisimétrica, $\mmm{A}^\trasp = -\mmm{A}$
...
\begin{figure}[ht]
  ...
  % Fondo
  \pgfmathsetmacro{\HORZ}{0.46}
  \pgfmathsetmacro{\VERT}{0.34}
  \pgfmathsetmacro{\MOD}{\HORZ^2 + \VERT^2}
  \pgfmathsetmacro{\ANGSD}{atan(\VERT / \HORZ)}
  \pgfmathsetmacro{\ANGII}{\ANGSD + 180.0}
  % 
