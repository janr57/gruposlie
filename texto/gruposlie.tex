...
La función sin derivar y su valor en $\varepsilon=\alpha/N = 0$ es
\[
  \mmm{R} (\alpha) = \left(\mmm{I} + \dfrac{\alpha}{N}\,\mmm{G}\right)^N
  ;\hspace{1em}
  \mmm{R} (0) = \mmm{I}
...
La primera derivada con respecto de $\alpha$ y su valor en $\varepsilon=\alpha/N=0$
\[
  \mmm{R}' (\alpha)
...
Entonces, el desarrollo en serie~\eqref{eq:gli-desarrolloserie}:
\begin{align*}
  \mmm{R} (\alpha) &= e^{\alpha\mmm{G}}
