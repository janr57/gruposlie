...
\label{sect:invariancia-bajo-rotaciones}
...
Así, la función rotada es idéntica a la original, ver figura~\ref{fig:mom-x2y2}, donde
...
Ahora vamos a rotar el vector de posición $\vvv{r}$ de una cierta partícula
\[
  \vvv{r} \longrightarrow \mmm{R}\vvv{r}
\]

Si derivamos el vector rotado $\mmm{R}\vvv{r}$ un ángulo $\theta$ fijo con respecto del
tiempo, tendremos que tener en cuenta que el ángulo es constante y que el vector posición
depende del tiempo
\[
  \dfrac{d}{dt}(\mmm{R}\vvv{r})
  = \dfrac{d}{dt}(\mmm{R}\left(\theta)\vvv{r}(t)\right)
  = \mmm{R}\left(\dfrac{d\vvv{r}(t)}{dt}\right)
  = \mmm{R}(\theta)\left(\vvv{v}(t)\right)
  = \mmm{R}\vvv{v}
\]
...
figura~\ref{fig:mom-giro-pos-momento}
\begin{align*}
  \vvv{r} &\longrightarrow \mmm{R}\vvv{r}\\
...
%    % Origen
  ...
  \caption{El vector momento lineal de una partícula gira el mismo ángulo que el
    vector posición.}
  \label{fig:mom-giro-pos-momento}
\end{figure}


\section{El momento angular y el teorema de Noether}
\label{sect:mom-momento-angular-y-Noether}
...
\ref{sect:invariancia-bajo-rotaciones}, si el lagrangiano fuera invariante bajo una
...
Para nuestro propósito en esta subsección, podremos utilizar tanto una rotación pasiva, como una activa, pues llegaríamos a la misma conclusión.
...
\ref{sect:mom-momento-angular-y-Noether}, también cambiarań de forma similar las
velocidades
\begin{align*}
  \vvv{\tilde{r}}
  &=
...
Se ha producido un cambio en la posición y en la velocidad
\begin{subequations}
...
La traspuesta de la parcial del lagrangiano con respecto a coordenadas cartesianas es
\[
  \left(\dfrac{\partial\symcal{L}}{\partial\vvv{r}}\right)^\trasp
  =
\begin{pmatrix}
