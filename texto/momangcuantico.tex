...
funciones en el espacio euclídeo. En esta vamos a extenderla al espacio de funciones
$L^2(\symbb{R}^3)$, esto es, funciones complejas de variable real en el espacio euclídeo.
...
punto del espacio $x, y, z$ un valor que puede ser complejo. El espacio de funciones que
...
rotar un punto (o una flechita) de coordenadas $(2,0,0)$, un ángulo de \ang{90} ---en
sentido antihorario---.
\begin{figure}[ht]
  \centering
  \begin{minipage}{0.40\linewidth}
    ...
    \begin{tikzpicture}[%
      ...
      \end{scope}
    \end{tikzpicture}
    \caption{Punto (o vector) $\vvv{P}$ rotado \ang{90} (rotación activa).}
    \label{fig:cua-funrot1}
  \end{minipage}
  \hspace{1em}
  ...
  % Escala
    ...
    \begin{tikzpicture}[%
      ...
      \end{scope}
    \end{tikzpicture}
    \caption{Otro punto (o vector) $\vvv{Q}$ rotado \ang{90} (rotación activa.}
    \label{fig:cua-funrot2}
  \end{minipage}
\end{figure}

%Recordemos que al aplicar una rotación pasiva, se transforman las coordenadas de todos
%los vectores de $\symbb{R}^3$, esto es, rotan los ejes que determinan todos los puntos
%del espacio. La matriz de rotación correspondiente es la representada en la
%ecuación~(\ref{eq:cua-rot-v1})
%\begin{equation}\label{eq:cua-matriz-rotacion-pasiva}
%  \mmm{R}(\xhat{n},\theta)
%  = \exp\left(i\theta\,\xhat{n}\cdot\mmm{J}\right)   
%\end{equation}
%Es importante señalar que en una rotación pasiva, rotan los ejes, pero no estamos rotando
%la función. Pero también se puede interpretar como si no hubieran girado los ejes pero la
%función lo hubiera hecho en sentido contrario. Así, podríamos suponer que los ejes no han girado y la función lo ha hecho en sentido contrario (horario).
%
%Aquí queremos rotar la función en sentido antihorario (rotación activa), de manera que
%debemos rotar los ejes en sentido horario, mediante la matriz inversa de
%\ref{eq:cua-matriz-rotacion-pasiva}. Como es la rotación que nos interesa, prescindiremos
%del exponente $-1$
%\[
%  \mmm{R}(\xhat{n},\theta)
%  = \exp\left(-i\theta\,\xhat{n}\cdot\mmm{J}\right)   
...
%  = \exp\left(-i\theta\,\xhat{n}\cdot\mmm{J}\right)   
%\]
%hemos de aplicar a las coordenadas de la función la matriz inversa
...
%\def\agl{60}
%Para centrar ideas y ayudarnos de la gráfica de funciones, vamos a suponer que nuestra función está definida en $\symbb{R}^2$, por lo que el eje perpendicular a $x$ e $y$
%representa el valor de la función y no el eje $z$.
%
%Supongamos que queremos girar una función $f(x,y)$, de dos variables independientes.
...
%$f(x,y) = -x e^{-(x^2+y^2)}$ un ángulo de $\ang{\agl}$ alrededor del eje $f(x,y)$
%\begin{figure}[ht]
%  \def\scl{0.9}
%  \def\agl{60}
%  \begin{tikzpicture}[%
...
Hemos estudiado el generador de rotación de vectores, pero ahora nos interesamos por el
generador de rotación de funciones.
Se usará el símbolo $\Psi(x,y,z)$ para representar cualquier función genérica; el símbolo
$\Psi$ se utiliza para que parezca una función de onda, pero por ahora para nosotros será
podrá ser cualquier función de las coordenadas. En realidad $\Psi(x,y,z)$ representa
cualquier función compleja de variable real.
...
$R(\varepsilon)=\exp(-i\varepsilon\,\xhat{n}\cdot\mmm{J})$, y como las rotaciones forman
un grupo de Lie, podemos aplicar rotaciones infinitesimales y las conclusiones a las que
lleguemos serán válidas para cualquier rotación finita
\[
  \mmm{R}(\varepsilon)
  =
...
Como $\mmm{J}$ es hermítica, la rotación inversa será la adjunta o traspuesta conjugada
\[
  \mmm{J}^\dagger = \mmm{J}
\]
