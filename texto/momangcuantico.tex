...
%$\xtilde{r} = (\tilde{x}, \tilde{y}, \tilde{z})$, señalado con tilde
%\begin{align*}
%  \xtilde{r}
%  &=
%  \mmm{R}_z(\theta) \vvv{r}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[%
    ...
    puntoR/.style={fill=green,draw=black,radius=1.4pt},
    puntoRt/.style={fill=red,draw=black,radius=1.4pt},
...
\begin{align*}%\label{eq:cua-desarrollo-psi}
  ...
  &=
    ...
    + \left(\frac{\partial\Psi(\vvv{r})}{\partial\vvv{r}}\right)^\dagger
    i\varepsilon\left(\xhat{n}\cdot\mmm{J}\right)\vvv{r}\\
\end{align*}
Simplificamos un poco la notación y pasamos $i\varepsilon$ hacia la izquierda
\[
  \tilde{\Psi}
  =
  \Psi
  + i\varepsilon
  \left(\dfrac{\partial\Psi}{\partial\vvv{r}}\right)^\dagger
  \left(\xhat{n}\cdot\mmm{J}\right)\vvv{r}
\]
Necesitamos que en el segundo sumando, la expresión anterior no tenga el complejo conjugado de la función, y que además se encuentre a la derecha. Todo ello se puede
conseguir realizando la transpuesta compleja conjugada de ese sumando. La transpuesta
es posible, pues ese sumando es un escalar (no es un vector ni matriz), y para la
operación complejo conjugado solo nos afecta la unidad imaginaria $i$, que cambiará
a $-i$.

Vayamos por partes, fijándonos en los siguientes detalles:
\begin{itemize}
\item La derivada parcial de $\Psi$ con respecto de $\vvv{r}$ es una matriz transpuesta
  compleja conjugada, es decir, es un vector fila $1\times 3$, y es el gradiente de
  $\Psi$, transpuesto conjugado
\[
  \left(\dfrac{\partial\Psi}{\partial\vvv{r}}\right)^\dagger
  =
  \begin{pmatrix}
...
\]
Si calculamos su traspuesta complejo conjugada, tendríamos un vector columna
\[
  \left[\left(\dfrac{\partial\Psi}{\partial\vvv{r}}\right)^\dagger\right]^\dagger
=
\
  \begin{pmatrix}
    \dfrac{\partial\Psi^*}{\partial x}
...
\item El ``producto'' $\xhat{n}\cdot\mmm{J}$ es una matriz cuadrada $3\times 3$,
  hermítica: $(\xhat{n}\cdot\mmm{J})^\dagger = \xhat{n}\cdot\mmm{J}$.
\item El vector $\vvv{r}$ es un vector columna $3\times 1$
  \[
    \vvv{r}
    =
    \begin{pmatrix}
...
%    =
%    \Psi\!\left(\vvv{r}\right)
%    + i\varepsilon\left(\frac{\partial\Psi(\vvv{r})}{\partial\vvv{r}}\right)^\dagger
%    \left(\xhat{n}\cdot\mmm{J}\right)\vvv{r}\\
%  &=
%    \Psi\!\left(\vvv{r}\right)
%    + \left[%
%    i\varepsilon\left(\frac{\partial\Psi(\vvv{r})}{\partial\vvv{r}}\right)^\dagger
%    \left(\xhat{n}\cdot\mmm{J}\right)\vvv{r}\right]^\dagger
%    =
%    \Psi\!\left(\vvv{r}\right)
%    + i^*\varepsilon\vvv{r}^\dagger \left(\xhat{n}\cdot\mmm{J}\right)^\dagger    \left[\left(\dfrac{\partial\Psi(\vvv{r})}{\partial\vvv{r}}\right)^\dagger\right]^\dagger\\
%  &=
%    \Psi\!\left(\vvv{r}\right)
%    - i\varepsilon\vvv{r}^\dagger\left(\xhat{n}\cdot\mmm{J}\right)
%    \dfrac{\partial\Psi(\vvv{r})}{\partial\vvv{r}}  
%\end{align*}
