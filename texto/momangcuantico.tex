...
Hemos visto el generador de rotación de vectores, pero ahora nos interesamos por la
...
\subsubsection{Ejemplo sencillo}
Pretendemos deducir cómo rotar una función utilizando un ejemplo sencillo.
En particular, queremos rotar la función $f(x,y) = 2x + 3y$, un ángulo de
$\ang{90} = \pi/2 \unit{\radian}$.

La matriz de rotación activa es
\[
  \mmm{R}(z, \pi/2)
  =
...
Así, las nuevas coordenadas $\vvv{\tilde{r}}$ de cualquier punto $\vvv{r} = (x, y, 0)$
del dominio de la función se calcularían
\[
  \vvv{\tilde{r}}
  =
  \mmm{R} \vvv{r}
...
Las nuevas coordenadas de cualquier punto $(x,y)$ del dominio de la función son
\vspace*{-1ex}
\begin{align*}
  \tilde{x} &= -y\\
  \tilde{y} &= x
...
\[
  \tilde{z} = z
\]
esto es, $\tilde{f}\!\left(\tilde{x},\tilde{y}\right) = f(x,y,z)$.

La función rotada que queremos es $\tilde{f}\!\left(\tilde{x},\tilde{y}\right)$ y la
expresión anterior nos señala la forma de calcularla.
