...
\ref{eq:cua-matriz-rotacion-pasiva}.
Pero como es la rotación que nos interesa, prescindiremos del exponente $-1$ y la
...
Sabemos que la matriz de rotación activa alrededor del eje $z$ es
\begin{align*}
  \mmm{R}((0,0,1), \pi/2)
  &=
...
Las nuevas coordenadas $\vvv{\tilde{r}}$ de cualquier punto $\vvv{r} = (x, y, z)$, se
calculan haciendo actuar la matriz de rotación anterior sobre este
\[
  \vvv{\tilde{r}}
  =
  \mmm{R} \vvv{r}
...
\ref{eq:cua-ejsenc-ytilde}, y la última \ref{eq:cua-ejsenc-ztilde}, se refiere a la
función rotada.
...
\begin{subequations}
  \begin{align*}
    ...
    \Psi(x,y) &\longrightarrow \tilde{\Psi}\!\left(\tilde{x},\tilde{y}\right)=\Psi(x,y) 
  \end{align*}
\end{subequations}
Esto es, cada punto del dominio $(x,y)$ rota, mientras que se lleva (arrastra consigo) a
la nueva posición $\left(\tilde{x},\tilde{y}\right)$, el valor que tenía la función
original en $(x,y)$.
...
\begin{equation}\label{eq:cua-ejsenc-funcion}
  ...
  2x + 3y
\end{equation}
Esta ecuación nos da la pista para obtener la expresión de la función rotada.
Sustituimos las coordenadas originales $x$ e $y$, del segundo miembro de la
igualdad anterior, por las nuevas $\tilde{x}$ e $\tilde{y}$.
...
\]
%\[
%  \tilde{\Psi}\!\left(\vvv{\tilde{r}}\right)
%  = \Psi\!\left(\mmm{R}^{-1}\vvv{\tilde{r}}\right)
%\]

En este caso, puesto que $\mmm{R}$ es una  matriz ortogonal, basta con calcular su
transpuesta
\[
  \mmm{R}^{-1}
  =
...
Vamos a comprobar que la función rotada es correcta, fijándonos en un punto del dominio
de la función original \eqref{eq:cua-ejsenc-funcion}. Por ejemplo, el punto representado
por el vector $\vvv{P} = (1,0)$. la función original da el resultado
\begin{equation}
...
%    vectorP/.style={%
%      -{Latex[width=7pt,length=8pt]},green!70!black,line width=2pt},
%    vectorPt/.style={%
%      -{Latex[width=7pt,length=8pt]},red!90!black,line width=2pt},
%    background/.style={%
%      line width=\bgborderwidth,
%      draw=\bgbordercolor,
%      fill=\bgcolor,
...
%    \draw[thick,->] (0,0,0) -- (\LONGXY,0,0) coordinate (ejex);
%    \node[below left,name=letraejex] at (ejex) {$x$};
%    \draw[thick,->] (0,0,0) -- (0,\LONGXY,0) coordinate (ejey);
%    \node[below right,name=letraejey] at (ejey) {$y$};
%    \draw[thick,->] (0,0,0) -- (0,0,\LONGZ) coordinate (ejez);
...
%    \draw[thick,->] (0,0,2.9) -- (0,0,\LONGZ);  
%    
%    % Vector de posición del punto P en gris
%    \draw[vectorP] (0,0,0) -- node[above left=0pt and -3pt] {\small $\vvv{r}$}
...
%    {\footnotesize $\tilde{\Psi}(\vvv{\tilde{r}}) = 2$};
%
%    % ÁNGULO PHI 
...
%    % Texto de rotación
%    \node[right=1.5em] at (0,0,3.6) {\small Rotación activa de};
%    \node[right=1.5em] at (0,0,3.2) {\small $\Psi= 2x+3y$};
%
%    %% PRUEBA
%    % P
%    \draw (P) -- ++(0,0,\Z) coordinate (Pvalor);
%    \draw (Pvalor) -- ++(-\RVEC,0,0);
%    % Ptilde
%    \draw (P') -- ++(0,0,\Z) coordinate (Ptvalor);
%    \draw (Ptvalor) -- ++(0,-\RVEC,0);
%    % Función
%    \filldraw[fill=green,draw=black] at (Pvalor) circle[radius=1.2pt];
%
%    % Fondo amarillo
%v    \coordinate (SW) at ($(current bounding box.south west) + (-\HORZ cm,-\VERT cm)$);
%    \coordinate (NE) at ($(current bounding box.north east) + (\HORZ cm,\VERT cm)$);    
%    \begin{scope}[on background layer]
%      \draw[background] (SW) rectangle (NE);
%    \end{scope}
%  \end{tikzpicture}
%  \caption{Rotación activa de la función $f(x,y,z) = 2x + 3y$, del ejemplo,
%    alrededor del eje $z$, un ángulo de \ang{90}, que se representa mediante la rotación
%    de un punto $P(1,0,2)$ hasta $\tilde{P}(0,1,2)$ del espacio tridimensional.}
...
\begin{figure}[ht]
  ...
  \begin{tikzpicture}[%
    ...
    % COORDENADAS
    \path (0,0,0) -- (\RVEC,0,0) coordinate (Px);
    \path (0,0,0) -- (0,\RVEC,0) coordinate (Pty);
    \path (Px) -- ++(0,0,\Z) coordinate (Pvalor);
    \path (Pty) -- ++(0,0,\Z) coordinate (Ptvalor);
    \path (Pvalor) -- ++(-\RVEC,0,0) coordinate (Pz);
    ...
    % PRUEBA
%    \filldraw[fill=black,draw=black] (Px) circle[radius=1.2pt];
%    \filldraw[fill=black,draw=black] (Pty) circle[radius=1.2pt];
%    \filldraw[fill=black,draw=black] (Pvalor) circle[radius=1.2pt];
%    \filldraw[fill=black,draw=black] (Ptvalor) circle[radius=1.2pt];
%    \filldraw[fill=black,draw=black] (Pz) circle[radius=1.2pt];

    % LÍNEAS GUÍA
    \draw[lineaguia] (Px) -- (Pvalor);
    \draw[lineaguia] (Pty) -- (Ptvalor);
    \draw[lineaguia] (Pvalor) -- (Pz);
    \draw[lineaguia] (Ptvalor) -- (Pz);
%    % Px
%    \draw (Px) -- ++(0,0,\Z) coordinate (Pvalor);
%    \draw (Pvalor) -- ++(-\RVEC,0,0);
%    % Ptilde
%    \draw (Pty) -- ++(0,0,\Z) coordinate (Ptvalor);
%    \draw (Ptvalor) -- ++(0,-\RVEC,0);
%    % Función
%    \filldraw[fill=green,draw=black] (Pvalor) circle[radius=1.2pt];
    ...
    \draw[vectorP] (0,0,0) -- coordinate (Pmed) (Pvalor);
    \filldraw[fill=green,draw=black] (Pvalor) circle[radius=1.2pt];
    \node[below left=0pt and -2pt, green!60!black] at (Pmed) {\footnotesize$\vvv{P}$};
    
    % Vector de posición del punto Ptilde en rojo
    \draw[vectorPtilde] (0,0,0) -- coordinate (Ptmed) (Ptvalor);
    \filldraw[fill=red,draw=black] (Ptvalor) circle[radius=1.2pt];
    \node[below right=0pt and -3pt, red!70!black] at (Ptmed) {\footnotesize$\xhat{P}$};
    
%    \path[vectorP] (0,0,0) -- node[above left=0pt and -3pt] {\small $\vvv{r}$}
%    (\RVEC,0,0) coordinate (P);
%    \filldraw[fill=gray, line width=0.4pt] (P) circle[radius=1.3pt];
%    \node[above right=-8pt and 0pt,black!50] at (P) {\small $P$};
%    \node[above left=0pt and 5pt] at (P)
...
%    {\footnotesize $\tilde{\Psi}(\vvv{\tilde{r}}) = 2$};
    ...
    \node[left=5pt, green!50!black] at (Pvalor) {\footnotesize $(1,0,2)$};
    \node[right=0pt, red!60!black] at (Ptvalor) {\footnotesize $(0,1,2)$};
    \filldraw (Px) circle[radius=0.8pt];
    \filldraw (Pty) circle[radius=0.8pt];
    \filldraw (Pz) circle[radius=0.8pt];
    \node[left] at (Px) {\scriptsize $1$};
    \node[above right=-2pt and -1pt] at (Pty) {\scriptsize $1$};
    \node[above left=-2.5pt and -1pt] at (Pz) {\scriptsize $2$};
    ...
    % Texto de rotación
    %\node[right=1.5em] at (0,0,3.6) {\footnotesize Rotación de \ang{90}};
    %\node[right=1.5em] at (0,0,3.2) {\footnotesize de  $\Psi= 2x+3y$};
    %\node[right=1.5em] at (0,0,2.8) {\footnotesize hasta $\tilde{\Psi}=-3x+2y$};    
