...
De esto se deduce que la inversa de una matriz unitaria es su autoadjunta, o traspuesta
compleja conjugada
\[
...
\begin{align*}
  e^{%
  \begin{pmatrix}
    \scriptstyle a & \scriptstyle 0\\ \scriptstyle 0 & \scriptstyle b\end{pmatrix}
  }
  &=
    \mmm{I}
    ...
    &=
      \begin{pNiceArray}{cc}
        1 + a + \frac{1}{2!}\,a^2+ \frac{1}{3!}\,a^3+\cdots & 0\\[0.5em]
        0 & 1 + b + \frac{1}{2!}\,b^2+\frac{1}{3!}\,b^3+\cdots
        \CodeAfter
        \tikz \draw[black!50,line width=0.4pt] (1.9-|1.0) -- (1.9-|last);
        \tikz \draw[black!50,line width=0.5pt] (2.0|-1.0) -- (2.0|-last);
      \end{pNiceArray}\\
    &=
      \begin{pmatrix}
        e^a & 0\\ 0 & e^b
...
\begin{equation}
  \label{eq:sux-determinante_U}
  \det(\mmm{U}(\theta))
  =
  e^{i\theta\kern1pt Tr\mmm{B}}
\end{equation}

De la expresión anterior deducimos que el determinante de $\mmm{U}(\theta)$ es un número
complejo, pues la traza de $\mmm{B}$ es un número real.
¿Qué módulo tiene el determinante de la matriz de transformación?
Podemos demostrar que vale la unidad, independientemente del valor de la traza de
$\mmm{B}$, (recordemos que el módulo de un complejo es, $|z| = \sqrt{z^*z}$)
\begin{align*}
  ...
  &=
  \sqrt{e^{-i\theta\kern1pt Tr\kern1pt\mmm{B}}
    \, e^{i\theta\kern1pt Tr\kern1pt\mmm{B}}
    \, e^{\frac{1}{2}(\theta \,Tr\mmm{B})^2\,[\mmm{B},\mmm{B}]}}
  =
  \sqrt{e^0\cdot e^0}
  = 1
\end{align*}

Pero nosotros no estamos interesados en el módulo del determinante de la matriz
unitaria, sino en el valor del determinante \eqref{eq:sux-determinante_U}, e imponemos
que su valor sea real e igual a $1$. Por tanto la traza de $\mmm{B}$ debe valer $0$.
...
Si además, renombramos las $x, y, z$ por $1, 2, 3$, respectivamente
\[
  \mmm{B}_{2\times 2}
  =
  \sqrt{a_1^2+a_{2}^2+a_{3}^{3}}
  \left(
    n_1\mmmg{\sigma_1} + n_2\mmmg{\sigma_2} + n_3\mmmg{\sigma_3}
  \right)
\]
con $n_1^2+n_{2}^2+n_{3}^2 = 1$, por lo que se puede considerar que las $n_{i}$ son las
componentes de un vector unitario $\xhat{n}$. En la siguiente expresión, y con las
debidas reservas utilizamos $\xhat{\vvv{n}}\cdot\mmm{\sigma}$ (aunque no es un producto
escalar) con el objetivo de abreviar la expresión matemática, mejor sería utilizar la
notación de Einstein, $n_i\sigma_i$, en lugar de $\xhat{\vvv{n}}\cdot\mmm{\sigma}$
\[
  \mmm{B}_{2\times 2}
  =
  \sqrt{a_1^2+a_{2}^2+a_{3}^2}\,\xhat{n}\cdot\mmm{\sigma}
\]
...
\[
  \mmm{U}(\theta)
  = e^{i\theta\mmm{B}}
  = e^{i\theta\sqrt{a_1^2+a_{2}^2+a_{3}^2}\,\xhat{n}\cdot\mmm{\sigma}}
\]

Llamamos $\beta = \theta\sqrt{a_1^2+a_2^2+a_3^2}$
\[
  \mmm{U}(\beta)
  = e^{i\beta\,\xhat{n}\cdot\mmm{\sigma}}
\]
...
El grupo SU(2) es
\[
  SU(2) = \left\{\mmm{U}(\theta) = e^{i\theta\,\xhat{n}\cdot \mmm{\sigma}}\right\}
\]
...
\begin{quote}
  \emph{SU(3) es el grupo de matrices $3\times 3$ complejas que se
...
\[
  \text{SU(3)}
  = \left\{\mmm{U}(\theta)
    = e^{i\theta\mmm{B}}\,/\, \mmm{B}^\dagger = \mmm{B}, Tr\,\mmm{B} =0,
...
Esta matriz ya es hermítica, $\mmm{B}^\dagger = \mmm{B}$ pero su traza no es nula.
Hay muchas maneras de organizar esta matriz para que se cumpla que la traza sea nula,
...
ecuación~(\ref{eq:sux-propiedad-producto-sigmas}).
Lo arregló reescalando la última matriz
{\small
  \begin{align*}
    \mmm{B}
    &=
      a_1\begin{pmatrix} 0 & 1/2 & 0\\ 1/2 & 0 & 0\\ 0 & 0 & 0 \end{pmatrix}
      + a_2\begin{pmatrix} 0 & -i/2 & 0\\ i/2 & 0 & 0\\ 0 & 0 & 0 \end{pmatrix}
      + a_3\begin{pmatrix} 1/2 & 0 & 0\\ 0 & -1/2 & 0\\ 0 & 0 & 0 \end{pmatrix}
      + a_4\begin{pmatrix} 0 & 0 & 1/2\\ 0 & 0 & 0\\ 1/2 & 0 & 0 \end{pmatrix}\\
    &+ a_5\begin{pmatrix} 0 & 0 & -i/2\\ 0 & 0 & 0\\ i/2 & 0 & 0 \end{pmatrix}
      + a_6\begin{pmatrix} 0 & 0 & 0\\ 0 & 0 & 1/2\\ 0 & 1/2 & 0 \end{pmatrix}
      + a_7\begin{pmatrix} 0 & 0 & 0\\ 0 & 0 & -i/2\\ 0 & i/2 & 0 \end{pmatrix}
      + a_8\begin{pmatrix}
        \frac{1}{2\sqrt{3}} & 0 & 0\\
        0 & \frac{1}{2\sqrt{3}} & 0\\
        0 & 0 & \frac{-1}{\sqrt{3}}
...
letra $\mmm{\lambda}$. Cualquier matriz del grupo SU(3) se puede expresar como
combinación lineal de estas
\[
  \mmm{B} = \sum_{i=1}^{8}a_i\mmm{\lambda}_i 
\]
...
Además se cumple
\[
  Tr\,(\mmm{\lambda}_i \mmm{\lambda}_j) = 2\delta_{ij}
\]
...
manera
\[
  [\mmm{\lambda}_i, \mmm{\lambda}_j]
  =
  i \sum_{k=1}^{8} f^{ijk}\,\mmm{\lambda}_k = i\kern1pt f^{ijk}\,\mmm{\lambda}_k
\]
...
$[\mmm{\lambda}_4,\mmm{\lambda}_5]$. Algunos coeficientes de estructura se pueden
calcular a partir de los conocidos, intercambiando índices. Los que no se pueden obtener
así se anulan
{\small
  \begin{align*}
    [\mmm{\lambda}_4, \mmm{\lambda}_5]
    &=
      i\kern1pt f^{45k}\mmm{\lambda}_k\\
    &=
      i\kern1pt\left[
      \cancelout{f^{451}}\mmm{\lambda}_1
      + \cancelout{f^{452}}\mmm{\lambda}_2
      + f^{453}\,\mmm{\lambda}_3
      + \cancelout{f^{454}}\mmm{\lambda}_4
      + \cancelout{f^{455}}\mmm{\lambda}_5
      + \cancelout{f^{456}}\mmm{\lambda}_6
      + \cancelout{f^{457}}\mmm{\lambda}_7
      + f^{458}\mmm{\lambda}_8
      \right]\\
    &=
      i\kern1pt\left[
      + f^{345}\mmm{\lambda}_{\kern1pt 3}
      + f^{458}\mmm{\lambda}_{\kern1pt 8}
      \right]\\
    &=
      i\kern1pt\left[\frac{1}{2}\,\mmm{\lambda}_{\kern1pt 3}
      + \frac{\sqrt{3}}{2}\,\mmm{\lambda}_{\kern1pt 8}\right]
  \end{align*}
}
